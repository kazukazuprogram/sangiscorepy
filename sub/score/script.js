...
function ch() {
  ...
  for (var x=0;x<9;x+=1){
    if (form[x].value=="") {
      ...
      nb += parseInt(form[x].value, 10)*1.2; // 社会
    } else if (x==3) {
      nb += parseInt(form[x].value, 10)*1.4; // 理科
    } else if (x>4) {
      nb += parseInt(form[x].value, 10)*1.2; // 実技科目
    } else {
      nb += parseInt(form[x].value, 10); // その他
    }
  }
  ...
  var writestr = "<table><tr><td><h3>調査書点</h3></td><td></td><td><h3>学力検査点</h3></td><td></td><td><h3>合計</h3></td></tr><tr><td><font size=\"8\">{0}</font>/300点</td><td><font size=\"4\">＋</font></td><td><font size=\"8\">{1}</font>/700点</td><td><font size=\"4\">＝</font></td><td><font size=\"8\">{2}</font>/1000点</td></tr></table>".replace("{0}",naishin).replace("{1}",exam).replace("{2}",score).replace("{3}",naa);
